...
int main(int argc, char **argv) {
  ...
  // generate vector u
  // nnz controlled by sparcity parameter // passed or hardcoded?
  float sparcity = VEC_SPARSITY;
  GrB_Vector_new(&u, GrB_FP64, nr);
  GrB_Vector_new(&w, GrB_FP64, nc);
  printf("# nnz in vector %lld\n", (long long)(nr * sparcity));
  for (int i = 0; i < nr * sparcity; ++i) {
    GrB_Index pos = rand() % nr;
    double val = (double)(rand() % nr) / (double)nr;
  ...
  for (int i = 0; i < N; ++i) {
    ...
    int res = GrB_vxm(w, NULL, NULL, TYPE, u, A, GrB_DESC_R);
