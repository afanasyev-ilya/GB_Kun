...
int main(int argc, char **argv) {
  ...
  lablas::Vector<float> w(nr);
  lablas::Vector<float> u(nc);
  auto nvals = (long long)(nc * sparcity);
  ...
  if (sparcity < 0.5) {
    ...
    for (int i = 0; i < nvals; ++i) {
      idxs[i] = rand() % nc;
      vals[i] = (float)(rand() % nc) / (float)nc;
    }
    std::cout << "# Should be sparce!\n";

    u.build(&idxs, &vals, nvals);
  } else {
    std::vector<float> vals(nc, 0);
    for (int i = 0; i < nc; ++i) {
      GrB_Index idx = rand() % nc;
      float val = (float)(rand() % nc) / (float)nc;
...
#define MASK_NULL static_cast<const lablas::Vector<float> *>(NULL)
    lablas::mxv(&w, MASK_NULL, NULL, lablas::PlusMultipliesSemiring<float>(),
                &A, &u, &desc);
#undef MASK_NULL
