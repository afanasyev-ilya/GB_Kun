...
def parse_arguments():
    ...
    parser.add_argument('--output',  dest='output', help='output file (default output_file)', default='output_file')

    return parser.parse_args()

def trans_graph(in_filename, out_filename, is_bin = False):
    edge_freqs = {}
    ...
    with tqdm.tqdm(total=os.path.getsize(in_filename), desc='Reading original file progress') as pbar:
        with open(in_filename) as file:
            for line in file:
                if "%" in line or "#" in line: # header line
                    print(line)
                else: # data line
...
def main():
    ...
    trans_graph(args.input, args.output)
