          ...
          Workspace *_workspace)
{
    ...
    for(VNT i = 0; i < _matrix->nrows; i++)
        reordered_x_vals[i] = x_vals[_matrix->col_new_to_old[i]];
    ...
    #pragma omp parallel
    {
        ...
        {
            reordered_x_vals[i - min_col] = x_vals[i];
        }*/
        ...
        /*#pragma omp for schedule(static)
        for(VNT idx = 0; idx < segment_data->size; idx++)
        {
            VNT row = idx;
            Y res = identity_val;
            for(ENT j = segment_data->row_ptr[row]; j < segment_data->row_ptr[row + 1]; j++)
            {
                ...
                res = add_op(res, mul_op(val, reordered_x_vals[col]));
            }
            shared_vector[row] = add_op(shared_vector[row], res);
        }*/
    }
